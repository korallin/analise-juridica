...
class STFDecMonocSpider(Spider):
    ...
    custom_settings = {
        "MONGO_COLLECTION": "decisoes_monocraticas",
        "ITEM_PIPELINES": {"acordaos.pipelines.MongoDBPipeline": 1},
    }
    ...
    def parseDoc(self, doc, response):
        ...
        if "Decisões no mesmo" in headers[-1]:
            ...
            if bodies[1][-1] == "\n":
                for i in range(1, len(bodies[-1]) - 1, 2):
                    bodies[-1][i] += "\n"
        ...
        law_fields_dict["similarDecisoes"] = parser.parseSimilarDecisoes(
    ...
    def urlPage(self, n):
        return (
            ...
            + "%29"
            + "+NAO+S.PRES."
            "&pagina=" + str(n) + "&base=baseMonocraticas"
        )
