...
def decisoes_dataquality(coll, lst, decision_type):
    ...
    write_count_list(lst, coll, {"$where": "this.cabecalho.length > 100"},
    ...
    write_count_list(lst, coll, {"$where": "this.tribunal == 'STF'"},
                    "tribunal", "Tribunal escrito corretamente")
    write_count_list(lst, coll, {"$where": "this.partesTexto.length > 100"},
                    "partesTexto", "Campo com tamanho mínimo válido")
    count = len([partes for partes in coll.find({},
                                  {"partes": 1}) if len(partes["partes"]) >= 3])
    ...
    write_count_list(lst, coll, {"$where": "this.legislacaoTexto.length > 100"},
    ...
    count = len([partes for partes in coll.find(
                                  {"$where": "this.partesTexto.length > 100"},
                                  {"partes": 1}) if len(partes["partes"]) >= 3])
    ...
    write_count_list(lst, coll, { "$and": [ { "$where": "this.legislacao.length > 0" },
                                { "$where": "this.legislacaoTexto.length > 100" } ] },
    ...
    write_count_list(lst, coll, { "$and": [ { "$where": "this.cabecalho.length > 100" },
                                            { "dataJulg": { "$type": "date" } },
                                            { "dataJulg": { "$gte": init_julg_date } } ] },
...
if __name__ == '__main__':
    ...
    df_decs_monocs.to_excel(writer, "decisões", index=False, startrow=3, startcol=3)
    df_acordaos.to_excel(writer, "decisões", index=False, startrow=3, startcol=10)
    writer.save()
